...
def api_import_leads():
    """
    Lets a script running on your own computer (like procore_finder.py)
    push leads straight in, without a browser login - it just needs to
    send the same secret shown on the Import Leads page. Every lead
    still goes through the normal duplicate check.
    """
    payload = request.get_json(silent=True) or {}
    if payload.get("secret") != config.API_SECRET:
